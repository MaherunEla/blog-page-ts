    ...
    <div className="bg-black ">
      <nav className="container flex flex-row items-center justify-between py-4">
        ...
        <div className="hidden md:flex flex-row items-center  gap-6 text-white">
          {navdata.map((item, index) => (
            <Link
              href={item.href}
              key={index}
              className="b1 font-inter text-white"
            >
              {item.page}
            </Link>
          ))}
          <button
            className="b1 py-2 text-white"
          ...
          <Link
            href="/"
            className="button font-bold leading-6 text-lg  text-black bg-white font-sen "
          >
            Subscribe
          </Link>
          {/* mobile icon */}
        ...
        <button onClick={handle} className="md:hidden flex items-right">
          {nav ? (
            <IoMdClose className="text-white w-10 h-10" />
          ) : (
            <SlMenu className="text-white w-10 h-10" />
          )}
        </button>
      </nav>
      {/* mobile navbar */}
      <div
        className={clsx(
          "md:hidden block h-[250px] bg-black",
          nav ? "block" : "hidden"
        )}
      >
        <div className="  flex flex-col items-center self-end">
          ...
          <Link href="/" className="button text-black bg-white font-sen">
            Subscribe
          </Link>
        </div>
      </div>
